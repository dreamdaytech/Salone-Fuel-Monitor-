...
// Note: Ensure you have GOOGLE_APPLICATION_CREDENTIALS set or the default credentials work
admin.initializeApp({
...
async function seedData() {
  ...
  // Optional: clear existing data if you want it to be idempotent or just append.
  // We will just append them or use monthYear as document ID
  
  for (const item of data) {
    const docRef = db.collection('price_trends').doc(item.monthYear.replace(' ', '_').toLowerCase());
    batch.set(docRef, {
      ...item,
